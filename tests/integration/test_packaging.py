...
def test_pyproject_declares_installable_solver_package():
    ...
    assert project["version"] == "0.2.4a0"
...
def test_solver_cli_version_runs():
    ...
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert completed.stdout.strip() == "QF_solver 0.2.4a0"
...
def test_portable_and_legacy_launchers_expose_qf_solver_identity():
    ...
    assert portable.stdout.strip() == "QF_solver 0.2.4a0"
    ...
    assert legacy.stdout.strip() == "QF_solver 0.2.4a0"
...
def test_mitc4_cli_version_runs():
    ...
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert "0.2.4a0" in completed.stdout

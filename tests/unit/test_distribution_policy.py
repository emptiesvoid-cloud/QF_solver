...
def test_distribution_policy_accepts_runtime_wheel_and_transparent_sdist(tmp_path: Path) -> None:
    wheel = tmp_path / "qf_solver-0.2.1a0-py3-none-any.whl"
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr("solveur/__init__.py", "")
        archive.writestr("qf_solver-0.2.1a0.dist-info/METADATA", "")
    sdist = tmp_path / "qf_solver-0.2.1a0.tar.gz"
    with tarfile.open(sdist, "w:gz") as archive:
        for relative in (
            "README.md",
            "LICENSE",
            "src/solveur/__init__.py",
            "src/solveur/compat/mitc4/__init__.py",
        ):
            payload = relative.encode("utf-8")
            info = tarfile.TarInfo(f"qf_solver-0.2.1a0/{relative}")

"""Run the independent reference and deterministic replay for WP09 HEX8 R2.

The structural primary results are pre-existing strict-tolerance diagnostic
evidence. This runner verifies their hashes, runs the independent NumPy
material-point/affine-element reference, and performs one fresh deterministic
production replay per H7/H8/H9 level. It never awards formal points.
"""
...
CONTRACT_PATH = ROOT / "qualification" / "0_2_9" / "wp09_hex8_formal_r2_contract.json"
...
def _load_runner_module():
    spec = importlib.util.spec_from_file_location("wp09_remesh_runner", RUNNER_PATH)
...
def _run_reference(output: Path) -> dict[str, Any]:
    ...
    independent_module = (
        "solveur.elements" not in module_text
        and "solveur.materials" not in module_text
        and "solveur.core" not in module_text
    )
    status = (
        "PASS_INDEPENDENT_REFERENCE"
        if completed.returncode == 0 and hex8 and hex8.get("status") == "PASS" and independent_module
        else "FAIL_CLOSED_INDEPENDENT_REFERENCE"
    )
    return {
        "status": status,
        ...
        "module_independence_check": "PASS" if independent_module else "FAIL",
        "hex8": hex8,
...
def _run_replay(
    ...
    for level in contract["mesh_hierarchy"]["levels"]:
        ...
        passed = (
            ...
            and all(value <= contract["gates"]["replay_relative_tolerance"] for value in checks.values())
        )
        if not passed:
        ...
        replay_payload = {
            ...
            "primary_raw_sha256": level["raw_sha256"],
            "metrics": metrics,
            "checks": checks,
        ...
        replay_path.write_text(
            json.dumps(replay_payload, indent=2, default=_json_default, allow_nan=False),
            encoding="utf-8",
        )
        rows.append(
            {
...
def _mesh_gate(contract: dict[str, Any], primary: dict[str, Any]) -> dict[str, Any]:
    middle = primary["M2"]["metrics"]
    fine = primary["M3"]["metrics"]
    names = {
        "selected_displacement": "selected_displacement_relative_max",
        "reaction_norm": "reaction_relative_max",
        "energy": "energy_relative_max",
        "von_mises_max": "von_mises_relative_max",
    }
    deltas = {name: _relative(float(middle[name]), float(fine[name])) for name in names}
    failures = [
        name
        for name, value in deltas.items()
        if value > contract["gates"]["mesh_comparison"][names[name]]
    ]
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "qualification" / "0_2_9" / "wp09_hex8_formal_r2_evidence")
    ...
    status = (
        ...
        and reference["status"] == "PASS_INDEPENDENT_REFERENCE"
        and replay["status"] == "PASS"
        and mesh["status"] == "PASS"
        else "FAIL_CLOSED"
    )
    campaign = {
        "status": status,
        "classification": "HEX8_FORMAL_CANDIDATE_REBOUND_NO_POINTS",
        ...
        "replay": replay,
        "mesh_gate_m2_to_m3": mesh,
        "formal_points": "0/8",
        "code_aster_status": "NOT_REQUIRED_INTERNAL_GATE",
    ...
    (output / "manifest.json").write_text(json.dumps({"schema_version": 1, "files": files}, indent=2), encoding="utf-8")
    report = [
        "# WP09 HEX8 R2 reference and replay evidence",
        "",
        f"Status: `{status}`",
        ...
        "Code_Aster is not required for this internal gate and was not used as evidence.",
        "",
        f"Independent reference: `{reference['status']}`",
        f"Replay: `{replay['status']}`",
        f"Mesh gate M2→M3: `{mesh['status']}`",
        "",

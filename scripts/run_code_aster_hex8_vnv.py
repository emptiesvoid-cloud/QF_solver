"""Run a same-mesh QF_solver HEX8 / Code_Aster HEXA8 static correlation."""
...
STUDY_ID = "VNV-HEX8-CODE-ASTER-HEXA8-STATIC-001"
...
def main() -> int:
    ...
    print(f"Code_Aster HEX8 correlation: {summary['status']}")
...
def run(output_dir: str | Path, *, image: str = DEFAULT_IMAGE) -> dict[str, Any]:
    ...
    load_node = next(index for index, point in enumerate(model.nodes) if np.allclose(point, [1.0, 0.0, 0.0]))
    stem = "hex8_hexa8"
    ...
    if completed.returncode != 0:
        raise RuntimeError("Code_Aster HEXA8 failed; see code_aster.log")
    ...
    summary: dict[str, Any] = {
        ...
        "external_solver": {"name": "Code_Aster", "version": "18.1.0", "image": image, "element": "3D/HEXA8"},
        "same_mesh": True,
        ...
        "limitations": [
            "One structured unit-cube mesh and one nodal-load linear static case are covered.",
            "The Code_Aster comparison is a complementary external correlation; modal and dynamic external cases remain outside this study.",
        ],
    }
...
def _mesh_text(nodes: Any, elements: Any, root: list[int], load_node: int) -> str:
    lines = ["TITRE", "QF_solver HEX8 same-mesh Code_Aster HEXA8 correlation", "FINSF", "COOR_3D"]
...
def _run_code_aster(work: Path, stem: str, image: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        ...
        capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=1200, check=False,
    )
...
def _report(summary: dict[str, Any]) -> str:
    ...
    return "\n".join([
        f"# {summary['study_id']}", "", f"Statut : **{summary['status']}**", "",
        "Meme maillage HEX8/HEXA8, memes BC, materiau et charge nodale.", "",
        "| Verification | Ecart | Limite | Statut |", "| --- | ---: | ---: | --- |",

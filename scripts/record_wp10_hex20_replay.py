"""Run and attest one fresh WP10-HEX20 replay process."""
...
def main() -> int:
    ...
    primary = out / "m3_replay.json"
    reference = out / "m3_reference.json"
    command = [
        sys.executable,
        str(root / "scripts" / "run_wp10_hex20_coupled.py"),
        "--case", "m2", "--output", str(primary), "--reference", str(reference)
    ]
    started = time.time()

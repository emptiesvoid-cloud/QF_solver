"""Bounded WP10 HEX8 corotational-J2/frictionless-contact campaign.

This runner is evidence tooling only.  It does not alter production mechanics,
thresholds or fallback policy.  The independent check recomputes contact
gap/penalty observables from the saved displacement path without importing the
production contact assembler; it is not an independent global FEM solve.
"""
...
def _model(with_contact: bool) -> tuple[FiniteElementModel, int]:
    base_model = _refinement_model("HEX8", 1)
    ...
    if with_contact:
        master = np.asarray(
            [[PLANE_X, -0.5, -0.5], [PLANE_X, 1.5, -0.5], [PLANE_X, 0.5, 1.5]], dtype=float
        )
    ...
    model = FiniteElementModel.from_raw(
        ...
        fixed_dofs=fixed,
        loads=[
            {"node": int(node), "dof": "UX", "value": 0.5 / len(loaded)} for node in loaded
        ],
        analysis={
        ...
        compact_steps.append(
            {
                ...
                "equivalent_plastic_strain_max": float(step.get("equivalent_plastic_strain_max", 0.0)),
            }
        )
    return {
        ...
        "max_relative_residual": max((row["relative_residual"] for row in compact_steps), default=0.0),
...
def independent_reference(primary: Path, output: Path) -> dict[str, Any]:
    ...
    if record["case"] == "M1":
        ...
        # The production contact convention is slave_position - plane_position.

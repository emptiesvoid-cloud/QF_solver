...
R3.3 exposed a second mesh-format constraint: node labels must be valid Code_Aster identifiers, not bare numeric tokens. The next exporter revision will use short alphabetic-prefixed names (`N1`, `N2`, …) for nodes and forces, retain alphabetic element names, and continue checking that every mesh record is at most 80 columns. The R3.3 contract, manifest, failed logs, and this audit remain preserved. The next campaign must use a new frozen contract and a completely new output root; no R3.3 raw result is reusable.

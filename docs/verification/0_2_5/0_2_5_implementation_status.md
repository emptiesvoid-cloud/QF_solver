...
est `PASS` selon sa semantique officielle de full regression. L'Owner a ensuite
accepte la baseline 0.2.4 avec limitation historique (`G00 PASS_WITH_HISTORICAL_LIMITATION`)
et approuve une revision explicite du scope : G04 et G06 sont
`NOT_IN_RELEASE_SCOPE` dans la matrice finale, sans devenir PASS ; leurs
qualifications restent experimentale/non qualifiee et differee. `025-G10` et
`025-G12` sont fermes uniquement pour ce scope qualifie borne.

## Etat par work package

| WP | Etat | Ce qui est effectivement disponible | Gate | Limite actuelle |
|---|---|---|---|---|
| WP0 | CLOSED_WITH_LIMITATION | Cartographie de l'architecture, contrats, tests cibles et replay historique isole identifies | 025-G00 PASS_WITH_HISTORICAL_LIMITATION | Un echec de readiness historique porte sur deux revisions RQ-G08 ; aucun defaut numerique demontre |
...
| WP4 | EXCLUDED_FROM_QUALIFIED_RELEASE | Correction arc-length augmentee sparse; cible signee et fenetre bornee `arc_length_stop_mode=max_steps` opt-in; chemin FEM TET4 total-Lagrangian sparse `VV-060` sur 24 pas; campagne interne monotone, restart depuis checkpoint intermédiaire, benchmark reduit shallow-arch traversant le point limite analytique avec erreur d'equilibre `< 1e-8`, chemin `total_lagrangian_j2` adaptatif borne sur TET4/TET10/HEX8/HEX20 `VV-064` vers le facteur 0,5 et télémétrie structurée des retries/cutbacks arc-length `VV-072` | 025-G04 NOT_IN_RELEASE_SCOPE | Les observations restent expérimentales : référence FEM indépendante et fermeture complète de branche non retenues dans le claim 0.2.5a0 |
...
| WP6 | EXCLUDED_FROM_QUALIFIED_RELEASE | Contrat `CompositeNonlinearAssembly` sparse et campagne interne rejouée sur un maillage TET4 connecté à deux éléments couvrant J2 + géométrie, géométrie + contact et J2 + géométrie + contact, complétée par les observations quatre familles TET4/TET10/HEX8/HEX20 et une étude de maillage J2 + géométrie niveaux 1/2/4, toutes via les chemins incrémentaux communs | 025-G06 NOT_IN_RELEASE_SCOPE | `results/vnv_0_2_5/g06_latest/` reste `PASS_INTERNAL_RESEARCH`; finite-kinematic J2 et corrélations externes couplées restent non qualifiés et différés |
...
| WP10 | CLOSED_BOUNDED | Archives externes 0.2.4 reutilisables et harnais Code_Aster multi-elements 0.2.5 execute sur maillage régulier partagé | 025-G10 PASS | G10 est évaluée uniquement sur les cellules MUST du scope final ; les cellules G04/G06 sont explicitement exclues et CalculiX reste SHOULD/non bloquant |
...
| WP12 | CLOSED_BOUNDED | Pack de planification, décision Owner de scope et readiness finale disponibles | 025-G12 PASS | Clôture agrégée limitée au scope approuvé ; tag, push et PyPI restent séparés |

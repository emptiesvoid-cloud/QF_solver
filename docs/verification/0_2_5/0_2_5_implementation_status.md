...
| WP1 | PARTIAL | J2 multi-elements connectes TET4/TET10/HEX8/HEX20, dissipation plastique, rollback, tangent FD, sensibilite coarse/reference/refined, raffinement regulier 1/2/4, chargement cyclique global, bilan energetique `Wext = Ue + Dp` et retry adversarial | 025-G01 OPEN | Corrélation Code_Aster régulière désormais concordante pour les quatre familles avec la règle TET10 explicite `code_aster_5`; acceptance bands, convergence/PEEQ et corrélation/qualification globale restent à fermer |
| WP2 | PARTIAL | Driver Full Newton commun pour la geometrie, TL StVK TET4/HEX8 et chemin expérimental `kinematics=total_lagrangian_j2` TET4/TET10/HEX8/HEX20 avec Green-Lagrange, seconde Piola, tangent sparse, objectivité et états aux points d'intégration; adaptateur géométrique sparse haut ordre couvert par un benchmark connecté TET10/HEX20 et accumulation CSR par chunks; couplage J2/geometrie connecté `VV-059` sur les quatre familles; smoke grande-déformation TET4/HEX8 avec angle de ligne d'extrémité supérieur à 0,5 radian; sensibilités maillage `VV-066` TET4/HEX8 et `VV-067` TET10/HEX20 à faible charge; composition TL géométrique + penalty contact `VV-065` via `CompositeNonlinearAssembly` | 025-G02 OPEN | Le modèle J2 finite-kinematic et l'adaptateur géométrique haut ordre restent des preuves internes de recherche, sans V&V externe, convergence de maillage acceptée ni qualification plastique en grande rotation; le contact géométrique reste un patch TET4 borné |
| WP3 | PARTIAL | Analyse publique `linear_buckling` bornée TET4/TET10/HEX8/HEX20 avec problème généralisé sparse `eigsh(K, M=-Kg)` lorsque `-Kg` est positif défini, raffinement shift-invert `eigs(K, M=-Kg)` après bracket sparse lorsque `-Kg` est indéfini, et fallback bracketé diagnostiqué si ARPACK refuse la paire; campagne interne facteurs/brackets, modes normalisés, résidus de mode et nnz sur les quatre familles; sensibilité coarse/medium sur maillage assemblé niveaux `1/2` pour les quatre familles; référence Euler TET4-TL rejouée sur les niveaux `24x6x6` et `32x8x8`; corrélation externe CalculiX TET4-TL/Euler passée sur quatre niveaux | 025-G03 OPEN | Les lignes TET10/HEX20 restent internes/recherche; la tendance coarse/medium n'est pas une convergence maillage; la corrélation externe de la route sparse haut ordre et le post-buckling restent hors fermeture |
...
| WP8 | PARTIAL | Script reproductible `scripts/benchmark_nonlinear_025.py` avec chemins explicites load-control, geometric_static, arc-length, finite-sliding borné, couplé et finite-kinematic arc-length; temps, DOF, iterations, allocations Python, RSS optionnelle, timers du driver Newton et décomposition élémentaire/sparse/contact; résumés multi-répétitions avec moyenne, médiane, min/max et écart-type; plan d'assemblage nonlinear reutilisant noyaux, matériaux immuables et mappings DDL avec compteurs hit/miss; cache de géométrie de référence Total-Lagrangian avec compteurs hit/miss; accumulation de tangente CSR par chunks avec compteurs de pic et estimation des buffers de staging; vectorisation exploratoire du tangent TL TET4/HEX8 conservée sans écart numérique observé | 025-G08 OPEN | Profil multi-répétitions sur SHA propre, comparaison numérique avant/après et caractérisation des chemins géométrique/contact restent à établir |
| WP9 | PARTIAL | Raisons structurees pour stagnation, contact, pénétration excessive, buckling, solveur lineaire, éléments invalides et etats non finis; `NumericalConvergenceError.to_dict()`, historique de residu Full Newton, compteur de line-search et campagne adversariale couvrant limites Newton, cutback minimum, élément invalide, mise à jour matériau, contact, pénétration, arc-length et bracket buckling; retry contact injecté, échec au deuxième incrément adaptatif, retry arc-length avec journal de rayon et cas réel `VV-068` de garde de pénétration avec rollback/cutback vérifiés | 025-G09 OPEN | La campagne agrégée est désormais `PASS_INTERNAL_FAILURE_CONTRACT`; les cas contact/path-dependent plus généraux, l'attachement à un SHA propre et la fermeture de gate restent à établir |
| WP10 | PARTIAL | Archives externes 0.2.4 reutilisables et harnais Code_Aster multi-elements 0.2.5 execute sur maillage régulier partagé | 025-G10 OPEN | Les quatre familles passent les historiques déplacement/réaction/contrainte/PEEQ à la tolérance 0,5 % avec conventions appariées; G10 reste ouvert pour les autres cellules externes 0.2.5 |
| WP11 | OPEN | Le dernier run ciblé complet archivé a produit `204 passed, 2 skipped` et un seul échec d'audit contrôlé; après synchronisation de l'audit, le test ciblé correspondant produit `2 passed` | 025-G11 OPEN | Le run ciblé complet n'a pas été relancé après ce correctif; suite complète, couverture et V&V globale restent hors de cette vérification |
...
- le kernel `total_lagrangian_j2` accepte désormais aussi TET10 et HEX20 dans
  ...
  `PASS_INTERNAL_RESEARCH` et ne ferme pas `025-G02` tant que l'objectivité,
  le tangent FD, la convergence de maillage et la corrélation externe des
  familles haut ordre ne sont pas produites.

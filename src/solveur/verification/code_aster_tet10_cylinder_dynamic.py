"""Code_Aster dynamic correlation for a circular TET10 cantilever."""
...
class CodeAsterTet10CylinderDynamicsCampaign(CodeAsterTet10DynamicsCampaign):
    """Run a quadratic circular-shaft TET10 cantilever against Code_Aster."""

    study_id = "VNV-TET10-DYNAMICS-CODEASTER-TETRA10-CYLINDER-023"
    ...
    def _spatial_mesh_sizes(self) -> tuple[float, float, float, float]:
        """Use ordered spatial levels for the circular quadratic solid."""
    ...
    def _model(
        ...
        *,
        ...
        mesh = BenchmarkMeshFactory().cylinder_tetra(
            self.output_dir / "meshes" / f"tet10_cylinder_h_{mesh_size:.3f}.msh",
            length=self.length,
            radius=0.4,
            mesh_size=mesh_size,
            order=2,
        ...
        if not root.size or not tip.size:
            raise RuntimeError("Circular TET10 mesh has no complete root or tip node group.")
        elements = [

...
def frictionless_contact_errors(model: "FiniteElementModel") -> list[str]:
    ...
    if model.analysis.type != "linear_static":
        errors.append("Frictionless contact is currently implemented for linear_static analysis only.")
    if model.analysis.method not in {"direct", "spsolve"}:

...
class TotalLagrangianTet4Assembly:
    ...
    def geometric_tangent(self, displacement: np.ndarray) -> csr_matrix:
        ...
        Linear buckling requires the geometric stiffness generated by the
        converged second-Piola stress.  It is not the difference between two
        complete material tangents, which would also include a material
        stiffness contribution.
        """
        local_displacement = self._local_displacements(displacement)
        _, stress = self._kinematics(local_displacement)
        scalar_blocks = np.einsum(
            "maJ,mJL,mbL->mab", self.gradients, stress, self.gradients, optimize=True
        )
        local_blocks = self.volumes[:, None, None, None, None] * np.einsum(
            "mab,ik->maibk", scalar_blocks, np.eye(3), optimize=True
        )
        local_tangent = local_blocks.reshape(-1, 12, 12)
        tangent = coo_matrix(
            (local_tangent.ravel(), (self._rows, self._cols)), shape=(self.ndof, self.ndof)
        ).tocsr()

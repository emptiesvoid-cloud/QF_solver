...
def _write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "## Modal",
        "",
        f"Fréquence fondamentale : `{modal['frequencies_hz'][0]:.6f}` Hz. Résidu relatif : `{modal['max_relative_residual']:.3e}`. Orthogonalités masse/raideur : `{modal['mass_orthogonality_error']:.3e}` / `{modal['stiffness_orthogonality_error']:.3e}`.",
        "",
        "## Newmark",
        "",
    ...
    lines.extend(
        [
            ...
            "## Harmonique",
            "",
            f"Erreur complexe maximale : `{harmonic['maximum_relative_error']:.3e}`. Limite statique à 0 Hz : `{harmonic['zero_hz_static_relative_error']:.3e}`. Post-traitement : `{harmonic['ply_count_at_first_frequency']}` plis.",
            "",
    ...
    lines.extend(["", f"Statut interne : **{summary['status']}**. Maturité : `{summary['maturity']}`.", ""])
...
def _plot_newmark(summary: dict[str, Any], path: Path) -> None:
    ...
    axis.plot(fine["times_s"], fine["reference_tip_uz_m"], "k--", label="référence modale")
    axis.plot(
...
def _plot_harmonic(summary: dict[str, Any], path: Path) -> None:
    ...
    axes[1].set_xlabel("fréquence / f1")
    for axis in axes:

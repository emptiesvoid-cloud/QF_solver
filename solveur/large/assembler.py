...
class ChunkedScipyAssembler:
    ...
    def assemble(self, model: LargeModel) -> ScipyLargeAssembly:
        stiffness = csr_matrix((model.ndof, model.ndof), dtype=float)
        for start in range(0, model.element_count, self.chunk_size):
            stop = min(start + self.chunk_size, model.element_count)
            stiffness = stiffness + self._chunk_matrix(model, start, stop)
    ...
    @staticmethod
    def _chunk_matrix(model: LargeModel, start: int, stop: int) -> csr_matrix:
        ...
        return coo_matrix((vals, (rows, cols)), shape=(model.ndof, model.ndof)).tocsr()
